...
export default function ExpensesGroup() {
  ...
  const handleAddMember = (e) => {
    ...
    setGroups((prev) =>
      prev.map((g) =>
        g.id === Number(selectedGroupId)
          ? { ...g, members: [...g.members, newMemberUsername] }
          : g
      )
    );
  ...
  const handleAddExpense = (e) => {
    ...
    const perPerson = Math.floor(   //This avoids decimals (like 83.33) and ensures each person pays an integer amount.
      Number(expenseAmount) / selectedGroup.members.length
    );
  ...
  return (
    <div className="p-6 dark:bg-blue-950 min-h-screen">
      ...
      <div className="bg-white dark:bg-gray-800 dark:text-white  p-6 rounded-xl shadow-lg">
        ...
        <div>
          {groups.map((g) => (
            <div key={g.id} className="border-b py-4">
                  ...
                  {selectedGroup.members.map((member) => (
                    ...
                    >
                      ...
                      <input
                        type="number"
                        placeholder="Amount"
                        className="w-1/3 p-1 border rounded-lg dark:bg-gray-700 dark:text-white"
                      />
                    </div>
                  ))}
...
export const Modal = ({ children, title, onClose }) => (
  <div className="fixed inset-0 bg-black bg-opacity-50 z-[100] flex justify-center items-center">
    <div className="bg-white dark:bg-gray-900 p-6 rounded-xl shadow-2xl w-full max-w-md  relative">
      <h3 className="text-xl font-bold mb-4 border-b pb-2 text-gray-800 dark:text-white">
        {title}
      </h3>
      <button
        onClick={onClose}
